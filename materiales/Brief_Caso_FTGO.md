...
Toma este brief como **única fuente oficial del dominio**: cualquier decisión arquitectónica que tomes debe poder rastrearse a este brief, a un capítulo del libro Richardson o a una de las *user stories* semilla. Inventar dominio fuera de aquí penaliza.
 ...
 | Repositorio oficial  | [github.com/microservices-patterns/ftgo-application](https://github.com/microservices-patterns/ftgo-application) |

### A.1 Contexto de negocio

FTGO es una plataforma de delivery que **conecta consumidores con restaurantes** ofreciendo entrega de comida a domicilio. El negocio ya está operando desde hace varios años como una **aplicación monolítica Java** empaquetada como WAR. El monolito ha acumulado los síntomas clásicos del *infierno monolítico* (Cap 1 del libro): builds lentos, escalado conflictivo entre módulos, falta de aislamiento de fallos, *lock-in* tecnológico y un equipo cada vez más bloqueado por el tamaño del código.
...
**Tu rol como maestro**: actuar como el arquitecto que produce los documentos iniciales de la nueva arquitectura (PRD + FSD + ADRs + diagramas C4) usando el caso FTGO como base. **No estás construyendo el sistema desde cero, estás documentando la arquitectura objetivo para migrar**.
...
### A.2 Stakeholders
 ...
 | Empleado FTGO (*back office*)    | Personal interno: *customer support*, finanzas, operaciones | Visibilidad, reportes, resolución de incidentes |
 | Equipo de arquitectura (tú)      | Responsable del rediseño hacia microservicios        | Calidad arquitectónica, trazabilidad, mantenibilidad |
...
Cada NFR de tu PRD debe poder rastrearse a una de estas restricciones (o a una nueva inferida y justificada).
 ...
 | Cumplimiento                | PCI-DSS para datos de pago (delegado a Stripe), GDPR/local para datos de consumidores                  |

### A.5 User stories semilla

Estas 3 *stories* son tu **punto de partida obligatorio**. En el FSD debes extenderlas a **≥ 5 UC** con bloques Given/When/Then y derivar al menos 2 UC adicionales justificables desde el *brief* o el libro (no inventados).
...
- Puede agregar/quitar *items* a su carrito.
...
- **Granularidad apropiada:** este es un PRD/FSD **ligero**, no exhaustivo. Cubre lo esencial, no agregues.
